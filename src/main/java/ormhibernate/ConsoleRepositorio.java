...
public class ConsoleRepositorio implements repositorio<Console> {
	...
	public void save(Console console) {
		/*
		 * session.beginTransaction(); System.out.println("hola" + user.nombre);
	...
	public void eliminarVideojuegoConsola(int id_consola, int id_videojuego) {
		Transaction transaction = null;
		try {
			transaction = session.beginTransaction();

			Console console = session.get(Console.class, id_consola);
			Videogames videogame = session.get(Videogames.class, id_videojuego);

			if (console != null && videogame != null) {
				console.removeVideogame(videogame);
